...
class Snoopy
{
     ...
     * @param mixed $formvars
     *            the formvars to use. format: $formvars["var"] = "val";
     * @param mixed $formfiles
     *            an array of files to submit format: $formfiles["var"] = "/dir/filename.ext";
    ...
    function _httprequest($url, $fp, $URI, $http_method, $content_type = "", $body = "")
    {
        ...
        if (($this->_framedepth < $this->maxframes) && preg_match_all("'<frame\s+.*src[\s]*=[\'\"]?([^\'\"\>]+)'i", $results, $match)) {
            ...
            $this->results[] = $results; // have we already fetched framed content?
                                             // no framed content
        else
            $this->results = $results;
        return $this;
        ...
        switch ($this->_submit_type) {
            case 'application/json':
                if (is_array($formvars)) {
                    $postdata = json_encode($formvars, JSON_UNESCAPED_UNICODE);
                } else {
                    $postdata = $formvars;
                }
                break;
